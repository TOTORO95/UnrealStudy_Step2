...
AABStageGimmick::AABStageGimmick()
{
	...
	StateChangeActions.Add(EStageState::REWARD,
		FStageChangedDelegateWrapper(FOnStageChangedDelegate::CreateUObject(this, &AABStageGimmick::SetChooseReward)));
	StateChangeActions.Add(EStageState::NEXT,
		FStageChangedDelegateWrapper(FOnStageChangedDelegate::CreateUObject(this, &AABStageGimmick::SetChooseNext)));
...
{
	// 에디터에서 변경되는 값을 적용할때 상태도 적용하게 확장
	...
	UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	bool bResult = GetWorld()->OverlapMultiByObjectType(OverlapResults, NewLocation, FQuat::Identity,
		FCollisionObjectQueryParams::InitType::AllStaticObjects, FCollisionShape::MakeSphere(775.0f), CollisionQueryParam);
...
{
	// switch로 관리해도되지만 늘어날수록 구조가 복잡해져 Delegate를 사용해 함수포인터로 관리
...
{
	IABGameInterface* ABGameMode = Cast<IABGameInterface>((UObject*)GetWorld()->GetAuthGameMode());
	if (ABGameMode)
	{
...
void AABStageGimmick::OnOpponentSpawn()
{
	...
	if (ABOpponentCharacter)
	{
		...
		FVector CurrentVector = ABOpponentCharacter->GetTransform().GetLocation();
		UE_LOG(LogTemp, Log, TEXT("Spawn Actor Pos %f %f %f"), CurrentVector.X, CurrentVector.Y, CurrentVector.Z);
	}
	else 
	{
		UE_LOG(LogTemp, Log, TEXT("Can't Spawn NonPlayerCharacter!!!"));	
	}
}
	...
	UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	{
		// Weak참조이기 때문에 객체의 유효성을 보장할수없기떄문에 체크하고 넘어가는게 좋다.
...
{
	// spawn -> OnComponentBeginOverlap
	// 오브젝트 자체의 델리게이트에의해서 이벤트가 발동하면 제어하기가 불편하다.
	// 오브젝트 자체의 델리게이트 설정을 미루고 그전에 정하게끔 변경
